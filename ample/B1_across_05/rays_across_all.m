...
ARR_TIME = zeros(nmod * 2,1);
V_EFF = zeros(nmod * 2,1);
RAYS_X = zeros(nmod * 2, size(x_r, 2));
RAYS_Y = zeros(nmod * 2, size(x_r, 2));
VGR = zeros(nmod * 2, size(x_r, 2));
RGS = zeros(nmod * 2, size(x_r, 2)-1);

tj_fixed = 0.5:0.5:6.5;
tj_fixed_len = length(tj_fixed);
tj_fixed_RGS = zeros(nmod * 2, tj_fixed_len);
TFIXED_X = tj_fixed_RGS;
TFIXED_Y = tj_fixed_RGS;
 
 for ii = 1:nmod
     disp(['VGM ' int2str(ii) ' of ' int2str(nmod)]);
     RAYS_X(ii * 2,:) =  x_r(n_a*(ii), :);
     RAYS_Y(ii * 2,:) = y_r(n_a*(ii), :);
     RAYS_X(ii * 2 + 1,:) =  x_r(n_a*(ii) + 5, :);
     RAYS_Y(ii * 2 + 1,:) = y_r(n_a*(ii) + 5, :);
    ...
    for ti = 1:tj_fixed_len
        ...
        r = tt_vgm * tt;
        tj_fixed_RGS(ii, ti) = r;

        r_abs = abs(RGS(ii, :) - r);
        rgs_closest_index = find(r_abs == min(r_abs));
        TFIXED_X(ii, ti) = RAYS_X(ii, rgs_closest_index);
        TFIXED_Y(ii, ti) = RAYS_Y(ii, rgs_closest_index);
    end
...
hold on
for ii = 1:5
    plot(RAYS_X(ii,:)/1000, RAYS_Y(ii,:)/1000, 'linewidth', 1, 'DisplayName', sprintf('Мода %i',ii));
...
for ii = 1:tj_fixed_len
    name = ['t = ', num2str(round(tj_fixed(ii), 1)), ' с'];
    plot(TFIXED_X(:, ii)/1000, TFIXED_Y(:, ii)/1000, 'Marker', '*', 'linewidth', 1.4, 'DisplayName', name);
    legend('-DynamicLegend');
end
