function RamsData = LoadConfigRAMS(pFolder, hydrologyFolder)

if nargin < 2
    hydrologyFolder = 'hydrology/';
end
...
dz = RamsData.dz;
drProf = RamsData.drProf;
dr = RamsData.dr;

save('Params.mat', 'dz', 'dr', 'drProf');
...
if exist([pFolder 'layers.txt'],'file')
    RamsData.cw = HydroLoadRAMS([pFolder hydrologyFolder], RamsData.dzHydr, RamsData.drProf, RamsData.rmax);
    RamsData.bLayers = LayersLoadRAMS(pFolder, RamsData.drProf, RamsData.rmax);    
else
    RamsData.cw = HydroLoadRAMS([pFolder hydrologyFolder], RamsData.dzHydr, RamsData.drProf);
